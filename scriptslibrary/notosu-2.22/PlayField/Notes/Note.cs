...
namespace StorybrewScripts
{
    ...
    public class Note
    {
        ...
        public Note(StoryboardLayer layer, OsuHitObject hitObject, Column column, double bpm, double offset, bool isMine = true)
        {
            this.isMine = true;
            this.layer = layer;
            this.hitObject = hitObject;
            ...
            var sprite = layer.CreateSprite("sb/sprites/mine.png");
        ...
        public void Render(double starttime, double endtime, OsbEasing easeing, double initialFade, double fadeInTime = 50, double fadeOutTime = 10)
        {
            ...
            if (isMine == true)
            {
                note.StartLoopGroup(starttime, 20);
                note.Rotate(0, 2000, 0, Math.PI * 2);
                note.EndGroup();

                note.Fade(starttime - 1, starttime + fadeInTime, 0, 0.35f);
               // note.Scale(starttime, 0.5f);
                note.Fade(endtime, 0);
                renderEnd = endtime;
            }
