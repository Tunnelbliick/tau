...
namespace StorybrewCommon.Storyboarding3d
{
    public class CameraState
    {
        ...
        public Vector4 ToScreen(Matrix4 transform, Vector3 point, double time = 0, string log = "test")
        {
            ...
            if (Math.Abs(transformedPoint.W) < 9f)
                transformedPoint.W = 9f;
            ...
            var depth = transformedPoint.Z / Math.Abs(transformedPoint.W);

            return new Vector4(screenPosition.X - offset, screenPosition.Y, depth, Math.Abs(transformedPoint.W));
        }
