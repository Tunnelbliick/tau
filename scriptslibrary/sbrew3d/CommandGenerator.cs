...
namespace StorybrewCommon.Storyboarding.Util
{
    public class CommandGenerator
    {
        ...
        {
            var previousState = null as State;
        ...
        private void convertToCommands(OsbSprite sprite, double? startTime, double? endTime, double timeOffset, bool loopable)
        {
            ...
            finalPositions.ForEachPair((start, end) => sprite.Move(start.Time, end.Time, start.Value, end.Value), new Vector2(320, 240),
                p => new Vector2(SafeRound(p.X, PositionDecimals), SafeRound(p.Y, PositionDecimals)), startStateTime, loopable: loopable);
            ...
            }, Vector2.One, s => new Vector2(SafeRound(s.X, ScaleDecimals), SafeRound(s.Y, ScaleDecimals)), startStateTime, loopable: loopable);
            finalRotations.ForEachPair((start, end) => sprite.Rotate(start.Time, end.Time, start.Value, end.Value), 0,
                r => SafeRound(r, RotationDecimals), startStateTime, loopable: loopable);
            ...
            finalOpacities.ForEachPair((start, end) => sprite.Fade(start.Time, end.Time, start.Value, end.Value), -1,
                o => SafeRound(o, OpacityDecimals), startStateTime, endStateTime, loopable: loopable);
        ...
        private float SafeRound(float value, int decimals)
        {
